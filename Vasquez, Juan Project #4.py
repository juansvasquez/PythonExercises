...
class HashTable:
        ...
        def __setitem__(self, dictKey, dictVal):
                hashBucket = self.buckets[self.hashFunction(dictKey)]      #LL
                search = hashBucket.search(dictKey)                        #Node
                if search != None:
                        if search.getKey() == dictKey:
                                search.setValue(dictVal)
                                return
                hashBucket.putFront(dictKey, dictVal)
        def __getitem__(self, dictKey):
                hashBucket = self.buckets[self.hashFunction(dictKey)]
                if hashBucket.search(dictKey) != None:
                        return hashBucket.search(dictKey).getValue()
                return None
